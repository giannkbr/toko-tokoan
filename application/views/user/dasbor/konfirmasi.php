...
// Form open
echo form_open_multipart(base_url('dasbor/konfirmasi/'.$detail_transaksi->kode_transaksi));
?>
...
<table class="table">
	<tbody>
		...
		<tr>
			<td>Tanggal Bayar</td>
			<td>
				<input type="text" name="tanggal_bayar" class="form-control" placeholder="dd-mm-yyyy" value="<?php if(isset($_POST['tanggal_bayar'])) { echo set_value('tanggal_bayar'); } elseif ($detail_transaksi->tanggal_bayar!="") { echo $detail_transaksi->tanggal_bayar; } else { echo date('d-m-Y'); } ?>">
			</td>
		</tr>
		<tr>
			<td>Jumlah Pembayaran</td>
			<td>
				<input type="number" name="jumlah_bayar" class="form-control" placeholder="Jumlah Pembayaran" value="<?php if(isset($_POST['jumlah_bayar'])) { echo set_value('jumlah_bayar'); } elseif($detail_transaksi->jumlah_bayar!=""){ echo $detail_transaksi->jumlah_bayar; } else{ echo $detail_transaksi->jumlah_transaksi; } ?>">
			</td>
		</tr>
		<tr>
			<td>Dari Bank</td>
			<td>
				<input type="text" name="nama_bank" placeholder="Nama Bank" class="form-control" value="<?php echo $detail_transaksi->nama_bank ?>">
		...
		<tr>
			...
			<td>
				<input type="text" name="rekening_pembayaran" placeholder="Nomor Rekening" class="form-control" value="<?php echo $detail_transaksi->rekening_pembayaran ?>">
		...
		<tr>
			...
			<td>
				<input type="text" name="rekening_pelanggan" placeholder="Nama Pemilik Rekening" class="form-control" value="<?php echo $detail_transaksi->rekening_pelanggan ?>">
				<small>contoh: Egi Harsono</small>
			</td>
		</tr>
		<tr>
