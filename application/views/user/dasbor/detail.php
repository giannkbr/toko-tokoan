...
<div class="p-b-10 m-l-21">
	<?php if ($detail_transaksi->status_bayar == 'Konfirmasi'){ ?>
		<h3 class="ltext-103 cl5">
			<?php echo $title ?> #<?php echo $detail_transaksi->kode_transaksi ?> <span style="background-color: #5cb85c; font-size: 10pt;" class="stext-101 cl0 size-101 bor1 p-lr-15">PAID</span>
		</h3>
	<?php } elseif ($detail_transaksi->status_bayar == 'Menunggu Konfirmasi') { ?>
		<h3 class="ltext-103 cl5">
			<?php echo $title ?> #<?php echo $detail_transaksi->kode_transaksi ?> <span style="background-color: #f0ad4e; font-size: 10pt;" class="stext-101 cl0 size-101 bor1 p-lr-15">PROCESS</span>
		</h3>
	<?php } elseif ($detail_transaksi->status_bayar == 'Belum') { ?>
		<h3 class="ltext-103 cl5">
			<?php echo $title ?> #<?php echo $detail_transaksi->kode_transaksi ?> <span style="background-color: #d9534f; font-size: 10pt;" class="stext-101 cl0 size-101 bor1 p-lr-15">UNPAID</span>
		</h3>
	<?php } ?>
</div>
<div class="row">
...
if ($detail_transaksi) { ?>
	...
	<table class="table table-bordered stext-107">
		...
		<tbody>
			...
			<tr>
				<td>Jumlah Total</td>
				<td>: Rp <?php echo number_format($detail_transaksi->jumlah_transaksi,0,',','.') ?>,-</td>
			</tr>
			<tr>
				<td>Status Bayar</td>
...
<h4 class="mtext-109 cl0">
	Rp <?php echo number_format($detail_transaksi->jumlah_transaksi,0,',','.') ?>,-
</h4>
	...
	<div id="div<?php echo $rekening2->id_rekening ?>" class="drop-down-show-hide">
		<p class="stext-102 cl0">
			...
			*Pastikan Anda sudah mentransfer uang sebelum mengeksekusi tombol. <br><br>
			...
			<span class="stext-110 cl0">INVOICE-<?php echo $detail_transaksi->kode_transaksi ?></span><br> 
			Pada kolom berita transfer.
		</p><br>
		<a href="<?php echo base_url('dasbor/konfirmasi/'.$detail_transaksi->kode_transaksi) ?>" class="flex-c-m stext-101 cl0 size-116 bg5 bor14 hov-btn2 p-lr-15 trans-04 pointer">
			Konfirmasi
		</a>
	</div>
	<?php endforeach ?>
